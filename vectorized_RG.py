...
def prob_move(var):
    # assumptions: the amount of pheromone has a set max and the average smell gene is constant
    ...
    for i in range(1,(var["food_max"]+1)):
        for j in range(0,(var["pher_max"]+1)):
            z[i,j] = var["smell_gene"][0]*(1-(i/var["food_max"])) + var["smell_gene"][0]*(j/var["pher_max"])
    plt.pcolormesh(x, y, z, cmap="Blues", shading="auto")
...
def stats_d(p2i, df):
    ...
    plt.legend()
    plt.xticks([20,33], ["more likely", "less likely"])
    plt.xlim(35,18)
    plt.ylim(0,1)
...
def stats_g(var, p2i, df):
    ...
    plt.bar(np.arange(var["pop_size"]), total, width=7)
